...
当前桌面版为 `0.1.0-preview.1`：它是未做 Authenticode 签名的 Windows x64 Technical Preview，不是生产发行版。用户必须校验发布 SHA-256，并应预期 Windows SmartScreen 或“未知发布者”提示。
...
- **Endpoint**：授权的私有或 loopback 服务保留在 Dock 后方。卖家配置本地 URL 与本地 `credentialRef`；Dock 执行无副作用的健康检查和 route 测试，并通过出站隧道服务已购买请求。本地 URL 与明文凭据不会进入公开 Listing。
- **API Bridge**：授权的公网 HTTPS 服务通过 Exora 透明 Gateway 调用。导入可以使用 OpenAPI 3.x，也可以使用经过卖家复核的结构化 route 草稿。Dock 与 Cloud 强制公网 HTTPS、DNS rebinding/重定向防护、安全 Header、健康检查、请求/响应边界和已声明计量。
...
V3.2 Alpha 不承诺：多租户 VM、MIG 分售、裸 Host Root、Hyper-V/VMware、WSL2 具备与 KVM 等价的隔离、未经卖家复核的自由文本 API 推断、外部商品金额结算、去中心化存储、平台模型推理、通用聊天界面、Agent 自主公开发布或自治争议裁决。未签名的 Windows Technical Preview 和本地配置的 Cloud 不代表生产可用性或资金托管认证。
