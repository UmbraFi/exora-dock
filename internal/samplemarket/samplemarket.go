...
func sampleSellerCard(dockID string, provider string) (agentcard.AgentCard, error) {
	...
	return agentcard.NewDraft(agentcard.DraftRequest{
		...
		Seller: agentcard.SellerManualFields{
			...
			StructuredByAgent:   "sample-agent",
			StructuredAt:        time.Now().UTC().Format(time.RFC3339),
			AllowedAgentActions: []string{"inspect accepted task inputs", "run approved inference workloads", "prepare quotes and deliverables"},
			ApprovalRequired:    []string{"use credential aliases", "write outside task outputs", "publish or spend funds"},
			CredentialPolicy:    "Credential aliases only; secret values stay in local owner-controlled storage.",
			NetworkPolicy:       "Network access is limited to endpoints declared for the accepted task.",
		},
