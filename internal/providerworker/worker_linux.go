...
func (s Server) provisionLease(ctx context.Context, in map[string]any) (map[string]any, error) {
	...
	leaseID := str(in, "leaseId")
	sshPublicKey := strings.TrimSpace(str(in, "sshPublicKey"))
	if sshPublicKey == "" {
	...
	metadata := map[string]any{"leaseId": leaseID, "secretUUID": secretUUID, "seedPath": seedPath, "domainPath": domainPath, "keyPath": keyPath, "diskPath": diskPath}
