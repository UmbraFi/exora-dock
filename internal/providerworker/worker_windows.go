...
func (s Server) dispatch(ctx context.Context, command string, input map[string]any) (map[string]any, error) {
	...
	case "lease_terminal_exec":
		return s.leaseTerminalExec(ctx, input)
	case "lease_workspace_stat":
		return s.leaseWorkspaceStat(input)
	case "lease_workspace_partial_stat":
		return s.leaseWorkspacePartialStat(input)
	case "lease_workspace_read":
		return s.leaseWorkspaceRead(input)
	case "lease_workspace_write":
		return s.leaseWorkspaceWrite(input)
	case "lease_transfer_review":
		return s.leaseTransferReview(input)
	case "lease_host_performance_probe":
		return s.leaseHostPerformanceProbe(ctx, input)
	case "lease_guest_performance_probe":
		return s.leaseGuestPerformanceProbe(ctx, input)
	case "lease_apply_load_throttle", "lease_clear_load_throttle":
		return s.leaseLoadThrottlePreview(input, command == "lease_apply_load_throttle")
	default:
...
func (s Server) validateImage(ctx context.Context, in map[string]any) (map[string]any, error) {
	...
	if _, err := s.Runner.Run(ctx, "wsl.exe", "-d", name, "--exec", "/usr/bin/install", "-d", "-m", "0755", "/run/sshd"); err != nil {
		return nil, fmt.Errorf("SSH runtime directory: %w", err)
	}
	_, _ = s.Runner.Run(ctx, "wsl.exe", "-d", name, "--exec", "/usr/bin/ssh-keygen", "-A")
	if _, err := s.Runner.Run(ctx, "wsl.exe", "-d", name, "--exec", "/usr/sbin/sshd", "-t"); err != nil {
...
func (s Server) wslDistributionExists(ctx context.Context, distro string) (bool, error) {
	out, err := s.Runner.Run(ctx, "wsl.exe", "--list", "--quiet")
	if err != nil {
		return false, err
	}
	for _, name := range strings.Fields(strings.ReplaceAll(out, "\x00", "")) {
...
func (s Server) provisionLease(ctx context.Context, input map[string]any) (map[string]any, error) {
	if winString(input, "securityMode") != "isolated_control_p2p_v1" {
	...
	imageVersion, _ := manifest["environmentImageVersion"].(string)
	artifact, _ := manifest["artifactPath"].(string)
	imageArtifactID := strings.TrimSpace(imageID)
	if version := strings.TrimSpace(imageVersion); version != "" {
		imageArtifactID += "-" + version
	}
	if artifact == "" && windowsSafeID.MatchString(imageArtifactID) {
		artifact = filepath.Join(s.DataDir, "images", imageArtifactID+".wsl")
	}
	artifactAbs, err := filepath.Abs(artifact)
	if err != nil {
		return nil, err
	...
	defer func() {
		...
		_, _ = s.Runner.Run(context.Background(), "wsl.exe", "--terminate", distro)
		_, _ = s.Runner.Run(context.Background(), "wsl.exe", "--unregister", distro)
		_ = os.RemoveAll(installPath)
	}()
	...
	hyperVFirewall := "Set-NetFirewallHyperVVMSetting -Name '{40E0AC32-46A5-438A-A0B2-2B479E8F2E90}' -DefaultInboundAction Block -DefaultOutboundAction Block -LoopbackEnabled `$false -ErrorAction Stop"
	if _, err := s.Runner.Run(ctx, "powershell.exe", "-NoProfile", "-NonInteractive", "-Command", hyperVFirewall); err != nil {
		return nil, fmt.Errorf("WSL Hyper-V isolation failed: %w", err)
	}
	workspacePath := filepath.Join(installPath, "workspace")
	if err := os.MkdirAll(workspacePath, 0700); err != nil {
		return nil, err
	}
	encodedWorkspace := base64.StdEncoding.EncodeToString([]byte(workspacePath))
	setupCommand := "umask 077; mkdir -p /workspace; mountpoint -q /workspace || mount -t drvfs \"$(echo " + encodedWorkspace + " | base64 -d)\" /workspace; chmod 700 /workspace"
	...
	metadata := map[string]any{"leaseId": leaseID, "leaseEpoch": winInt64(input, "leaseEpoch"), "distro": distro, "installPath": installPath, "workspacePath": workspacePath, "securityMode": "isolated_control_p2p_v1", "hyperVFirewall": true, "workloadPolicy": input["workloadPolicy"], "performancePolicy": input["performancePolicy"], "enforcementMode": input["enforcementMode"], "guestProbePublicKey": guestProbePublicKey}
	...
	capability := map[string]any{"protocol": "exora_control", "terminal": true, "fileTransfer": "webrtc_p2p", "networkAccess": "none", "workspace": "/workspace", "leaseEpoch": winInt64(input, "leaseEpoch")}
	provisioned = true
	return map[string]any{"leaseId": leaseID, "state": "active", "guestVerified": true, "backend": "wsl2", "isolationClass": "managed_wsl2_shared_host", "resourceDisclosure": map[string]any{"singleLeasePerHost": true, "cpuMemoryAreConfiguredCaps": true, "gpuSharedWindowsDriver": true, "hardwarePassthroughExclusive": false, "networkAccess": "none"}, "capability": capability, "guestProbePublicKey": guestProbePublicKey}, nil
}

func winBoundedBPS(value int64) int64 {
	if value < 0 {
		return 0
	}
	if value > 10000 {
...
func firstWindowsNumber(value string) int64 {
	match := regexp.MustCompile(`([0-9.]+)`).FindStringSubmatch(value)
	if len(match) != 2 {
		return 0
	}
...
func (s Server) leaseHostPerformanceProbe(ctx context.Context, input map[string]any) (map[string]any, error) {
	if _, err := s.isolatedLeaseMetadata(input); err != nil {
		return nil, err
	}
	cpuOut, cpuErr := s.Runner.Run(ctx, "powershell.exe", "-NoProfile", "-NonInteractive", "-Command", "(Get-Counter '\\Processor(_Total)\\% Processor Time').CounterSamples.CookedValue")
	if cpuErr != nil {
		return nil, fmt.Errorf("performance_probe_unavailable: %w", cpuErr)
	}
	gpuOut, _ := s.Runner.Run(ctx, "nvidia-smi.exe", "--query-gpu=utilization.gpu,clocks.sm,clocks.max.sm,temperature.gpu,clocks_throttle_reasons.active", "--format=csv,noheader,nounits")
	cpuBPS := winBoundedBPS(firstWindowsNumber(cpuOut) * 100)
	gpuBPS, clockBPS := int64(0), int64(10000)
	thermal, power := false, false
	if line := strings.TrimSpace(strings.Split(gpuOut, "\n")[0]); line != "" {
		fields := strings.Split(line, ",")
		if len(fields) > 0 {
			gpuBPS = winBoundedBPS(firstWindowsNumber(fields[0]) * 100)
		}
		if len(fields) > 2 {
			maximum := firstWindowsNumber(fields[2])
			if maximum > 0 {
				clockBPS = winBoundedBPS(firstWindowsNumber(fields[1]) * 10000 / maximum)
			}
		}
	...
	return map[string]any{"host": host, "sampledAt": time.Now().UTC(), "privacy": "aggregate_only", "enforcementMode": "monitor_only_preview"}, nil
}

func (s Server) leaseGuestPerformanceProbe(ctx context.Context, input map[string]any) (map[string]any, error) {
	metadata, err := s.isolatedLeaseMetadata(input)
	if err != nil {
		return nil, err
	}
	leaseID := winString(metadata, "leaseId")
	...
	if runErr == nil {
		...
		if elapsed > 2*time.Second {
			score = winBoundedBPS(int64(2*time.Second) * 10000 / int64(elapsed))
		}
...
func (s Server) leaseLoadThrottlePreview(input map[string]any, apply bool) (map[string]any, error) {
	metadata, err := s.isolatedLeaseMetadata(input)
	if err != nil {
		return nil, err
	}
	return map[string]any{"leaseId": winString(metadata, "leaseId"), "throttled": false, "requested": apply, "enforcementMode": "monitor_only_preview", "disclosure": "Windows/WSL2 Preview records sustained-load events but does not enforce host throttling"}, nil
}

func (s Server) leaseSSHTarget(input map[string]any) (map[string]any, error) {
	leaseID := winString(input, "leaseId")
	if _, err := windowsLeaseDistro(leaseID); err != nil {
		return nil, err
	}
	raw, err := os.ReadFile(s.wslLeaseLockPath())
	if err != nil {
		return nil, fmt.Errorf("active lease metadata unavailable: %w", err)
	}
	metadata := map[string]any{}
	if err := json.Unmarshal(raw, &metadata); err != nil {
		return nil, errors.New("invalid active lease metadata")
	}
	if current, _ := metadata["leaseId"].(string); current != leaseID {
		return nil, errors.New("lease is not active on this provider")
	}
	if epoch := winInt64(input, "leaseEpoch"); epoch <= 0 || epoch != winInt64(metadata, "leaseEpoch") {
		return nil, errors.New("stale leaseEpoch")
	}
	host, _ := metadata["guestIP"].(string)
	if net.ParseIP(strings.TrimSpace(host)) == nil {
		return nil, errors.New("managed guest address is unavailable")
	}
	return map[string]any{"host": host, "port": 22}, nil
}

func (s Server) isolatedLeaseMetadata(input map[string]any) (map[string]any, error) {
	leaseID := winString(input, "leaseId")
	if _, err := windowsLeaseDistro(leaseID); err != nil {
		return nil, err
	}
	...
	metadata := map[string]any{}
	if json.Unmarshal(raw, &metadata) != nil || winString(metadata, "leaseId") != leaseID || winString(metadata, "securityMode") != "isolated_control_p2p_v1" {
...
func (s Server) leaseTerminalExec(ctx context.Context, input map[string]any) (map[string]any, error) {
	metadata, err := s.isolatedLeaseMetadata(input)
	if err != nil {
		return nil, err
	}
	command := winString(input, "command")
	if strings.TrimSpace(command) == "" || len(command) > 64<<10 {
		return nil, errors.New("terminal command must contain 1-65536 bytes")
	}
	distro := winString(metadata, "distro")
	output, runErr := s.Runner.Run(ctx, "wsl.exe", "-d", distro, "--user", "root", "--exec", "sh", "-lc", command)
	if runErr != nil {
		return map[string]any{"stdout": output, "stderr": runErr.Error(), "exitCode": 1}, nil
	}
	return map[string]any{"stdout": output, "stderr": "", "exitCode": 0}, nil
}

func (s Server) leaseWorkspaceRoot(input map[string]any) (string, error) {
	metadata, err := s.isolatedLeaseMetadata(input)
	if err != nil {
		return "", err
	}
	root := winString(metadata, "workspacePath")
	if root == "" {
...
func (s Server) leaseWorkspaceStat(input map[string]any) (map[string]any, error) {
	root, err := s.leaseWorkspaceRoot(input)
	if err != nil {
		return nil, err
	}
	return workspaceStatResult(root, winString(input, "relativePath"))
}

func (s Server) leaseWorkspacePartialStat(input map[string]any) (map[string]any, error) {
	root, err := s.leaseWorkspaceRoot(input)
	if err != nil {
		return nil, err
	}
	return workspacePartialStatResult(root, winString(input, "relativePath"))
}

func (s Server) leaseWorkspaceRead(input map[string]any) (map[string]any, error) {
	root, err := s.leaseWorkspaceRoot(input)
	if err != nil {
		return nil, err
	}
	return workspaceReadResult(root, winString(input, "relativePath"), workerInt64(input["offset"]), workerInt64(input["limit"]))
}

func (s Server) leaseWorkspaceWrite(input map[string]any) (map[string]any, error) {
	root, err := s.leaseWorkspaceRoot(input)
	if err != nil {
		return nil, err
	}
	finalize, _ := input["finalize"].(bool)
	return workspaceWriteResult(root, winString(input, "relativePath"), winString(input, "dataBase64"), workerInt64(input["offset"]), finalize, workerInt64(input["expectedSize"]), winString(input, "expectedSha256"))
}
...
func (s Server) resetLease(ctx context.Context, input map[string]any) (map[string]any, error) {
	...
	port := int(winInt64(metadata, "portProxyPort"))
	if port == 0 {
		if accessMode, _ := metadata["accessMode"].(string); accessMode == "" {
	...
	exists, err := s.wslDistributionExists(ctx, distro)
	if err != nil {
